...
def GenObservatory():
	...
	if os.path.isfile( os.path.expanduser("~")+"/.rmob-export/backup.conf"):
		print "Did you like recover last Observatory's setting from "
		read = open( os.path.expanduser("~")+"/.rmob-export/backup.conf" , 'r')
		...
		while var != "Yes" and var != "no":
			print "You must push 'Yes' or 'no'"
			var = raw_input("Try it again: ")
		if var == "Yes":
			i=0
			read = open(os.path.expanduser("~")+"/.rmob-export/backup.conf", 'r')
			for line in iter(read):
...
def Backup():
	print "Did you like backup this setting?"
	var = raw_input("'Yes'/'no': ")
	while var != "Yes" and var != "no":
		print "You must push 'Yes' or 'no'"
		var = raw_input("Try it again: ")
	if var == "Yes":
		...
		dir = home+"/.rmob-export"
		try:
			os.stat(dir)
		except:
			os.mkdir(dir)
		shutil.copy2('./io/Observatory.info', home+'/.rmob-export/backup.conf')
		print "Zaloha byla dokoncena"
	elif var == "no":
		print "Konec Configure.py"
